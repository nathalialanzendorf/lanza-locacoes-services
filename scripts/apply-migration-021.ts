/**
 * Aplica packages/db/sql/021_vendas.sql no Postgres (OIDC / .env.local).
 * Uso: npx tsx scripts/apply-migration-021.ts
 */
...
async function main(): Promise<void> {
  ...
  const sqlPath = resolve(process.cwd(), "packages/db/sql/021_vendas.sql");
  const sql = readFileSync(sqlPath, "utf8");

  console.log("Aplicando 021_vendas.sql …");
  await pool.query(sql, undefined, "migration-021");
  ...
  console.log("tabela vendas existe:", check.rows[0]?.exists === true);
}
