 ...
 *   POST http://127.0.0.1:9237/capture/start  { apiUrl, bearer?, apiKey? }
 */
...
async function persistToRemoteApi(
  ...
  const r = await fetch(`${base}/api/portais/detran-rs/sessao`, {
    method: "PUT",
    headers,
    body: JSON.stringify({ auth: session.auth, userId: session.userId }),
  });
  if (!r.ok) {
...
const server = createServer(async (req, res) => {
  ...
  try {
    if (req.method === "GET" && path === "/capture/status") {
      json(res, 200, { data: getDetranRsCaptureState() });
      return;
    }
    ...
    if (req.method === "DELETE" && path === "/capture/stop") {
      const data = await stopDetranRsCapture();
      json(res, 200, { data });
      return;
    }
