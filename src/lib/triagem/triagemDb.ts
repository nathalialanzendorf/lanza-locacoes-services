/**
 * Database do histórico de triagens de locatário (`database/triagem.json`).
 *
 * Cada execução da skill `triagem-locatario` grava aqui um registro com os
 * dados do locatário, a base legal (LGPD), o resultado por fonte e os caminhos
 * dos relatórios gerados. Mantém histórico (uma triagem por dia, por CPF).
 *
...
export const DB_TRIAGEM = path.join(REPO_ROOT, "database", "triagem.json");
...
const DEFAULT_DESCRICAO =
  "Histórico de triagens de locatário (antecedentes criminais / processos). id = uuid; chave natural cpf + dataConsulta (1 por dia). clienteId -> clientes.json. Detalhe completo (achados) fica nos relatórios em relatorios/triagem/. Gerado pela skill triagem-locatario; NÃO contém dados de terceiros sem base legal registrada (LGPD).";
...
const DEFAULT_SCHEMA: Record<string, string> = {
  ...
  dataConsulta: "AAAA-MM-DD — data da triagem (chave natural com cpf)",
  lgpd: "{ baseLegal, titularConsentimento, solicitante, finalidade }",
  alertaGeral: "true se alguma fonte sinalizou risco",
  resumo: "Conclusão automática (texto)",
  fontes:
    "[] resumo por fonte: { id, nome, status, alerta, observacao, qtdAchados, evidencia, consultadoEm }",
  relatorioJson: "Caminho (relativo ao repo) do relatório .json completo",
  relatorioMd: "Caminho (relativo ao repo) do resumo .md",
...
export function registrarTriagem(args: {
  ...
  const base: Omit<TriagemRegistro, "id" | "cadastradoEm"> = {
    ...
    relatorioMd: relativo(args.caminhoMd),
    atualizadoEm: ts,
  };
