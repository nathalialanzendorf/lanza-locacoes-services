...
description: >-
  Registers rental receipts on rastreame.com.br under Gastos Gerais (tipo
  OUTROS): Monday ATRASADO placeholders for weekly payments, full or partial
  settlement, weekly invoices, duplicate check on info before POST. Use when
  the user asks cadastrar recebimento, recebimento locação rastreame, rastreame
  gastos gerais, pagamento semanal locação, fatura semanal motorista, controle
  recebimento locatário, or gastos/list.
---

# Cadastrar recebimento (Rastreame — Gastos gerais)

Skill para **controlo de recebimento** dos clientes que locam o veículo via **Rastreame**: cadastrar **pagamentos** e **faturas da semana** na área **Gastos Gerais**, alinhar valores ao **contrato** e **veículo** em `database/` quando existir, e tratar **pagamento integral** vs **parcial**.
...
- **Auth** (`RASTREAME_*`) e **comandos** no site: skill **rastreame-site** (única referência técnica).
...
**Automatização:** agendar na máquina do operador (ex.: **Agendador de Tarefas** do Windows) um comando à segunda que invoque o agente com esta skill **ou** scripts que chamem os comandos da skill **rastreame-site** (`rastreame-gastos post`, etc.) com JSON gerado (sempre após verificação de duplicados).
...
1. Confirmar autenticação (skill **rastreame-site**).
2. Identificar motorista (`motorista.key`) e veículo (`rastreavel.key`) via UI ou `rastreame-gastos list` (skill **rastreame-site**). Código de referência: `src/lib/rastreame/motorista.ts`, `src/lib/rastreame/gasto.ts`.
3. **Antes de qualquer `POST`:** verificar duplicado por **`info`** + motorista + rastreável (secção **Antes de cadastrar**).
4. Para rotina de segunda: listar quem deve parcela → só então **`rastreame-gastos post`** (skill **rastreame-site**) com título **`ATRASADO - Pagamento semanal - …`** e `total` / `data` 23:59 (se já existir o mesmo `info` para o par, não duplicar).
5. Para quitação: localizar gasto na [listagem](https://rastreame.com.br/#/gastos/list) → aplicar regra integral vs parcial: **`rastreame-gastos put`** e, se parcial, **`rastreame-gastos post`** (skill **rastreame-site**), com verificação de duplicado antes do `post`.

## Dependências

- Skill **rastreame-site** — execução dos comandos no site.
