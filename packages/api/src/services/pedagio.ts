...
export async function listarVeiculosPortal() {
  const items = await listarVeiculosPedagio();
...
export async function registrarVeiculoPortal(input: {
  ...
  const modelo =
    input.modelo?.trim() ??
    (local ? modeloComposto(local) : null);
  if (!modelo) {
    throw new HttpError(
      400,
      "Modelo não informado e veículo não encontrado em veiculos.json",
    );
  }

  const r = await registrarPlaca({
    placa,
    modelo,
    marca: input.marca ?? local?.marca,
    ano: input.ano ?? local?.ano ?? local?.anoModelo?.split("/")[0],
    cor: input.cor ?? local?.cor,
  });
  if (!r.ok) {
...
export async function excluirVeiculoPortal(placa: string, dryRun = false) {
  ...
  if (dryRun) {
    const lista = await listarVeiculosPedagio();
    ...
    for (const p of faltam) {
      ...
      try {
        const r = await registrarPlaca({
          ...
          modelo,
          marca: local?.marca,
          ano: local?.ano ?? local?.anoModelo?.split("/")[0],
          cor: local?.cor,
        });
        registrados.push({
          placa: r.placa,
