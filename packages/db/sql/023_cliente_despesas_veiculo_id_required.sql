-- cliente_despesas: veiculo_id obrigatorio; remove coluna redundante veiculo_placa.
...
BEGIN
  ...
  IF orphan_count > 0 THEN
    RAISE EXCEPTION
      'cliente_despesas: % linha(s) sem veiculo_id resolvivel — corrija antes de migrar',
      orphan_count;
  END IF;
END $$;
...
COMMENT ON COLUMN lanza.cliente_despesas.veiculo_id IS
  'FK UUID → lanza.veiculos.id (obrigatorio; placa via JOIN).';
